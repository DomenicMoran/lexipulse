...
- [ ] **App-Privacy-Antwort fuer die mobile App gegenpruefen** — Emulator-Durchgang 2026-08-23 (`EMULATOR-LEXIPULSE-2026-08-23.md`) fand per Netzwerkmitschnitt und Quelltextpruefung, dass die mobilen Apps beim URL-Import die eingegebene Adresse direkt vom Geraet abrufen (kein `/api/extract`, das nutzt nur die Web-App) — der Betreiber der fremden Seite sieht dabei die Geraete-IP. `store/metadata/app-privacy.md` und `store/legal/datenschutz.de.md`/`privacy.en.md` wurden auf diesen tatsaechlichen Mechanismus korrigiert. Die Schlussfolgerung „No, we do not collect data from this app" in App Store Connect duerfte weiterhin halten (die App selbst speichert nichts, sie leitet nur technisch weiter wie ein Browser), aber das ist eine Rechtseinschaetzung — bitte vor der naechsten Apple-Einreichung einmal gegenlesen.
